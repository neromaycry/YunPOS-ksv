...
define([
    ...
    '../../moduals/modal-brecardlogin/model',
    '../../moduals/modal-brecardlogin/collection',
    '../../moduals/modal-brecardpay/view',
    'text!../../moduals/modal-brecardlogin/tpl.html',
], function (BaseModalView,BREcardModel,BREcardCollection,BREcardView, tpl) {

    var brecardView = BaseModalView.extend({
        ...
        modalInitPage: function () {
            this.model = new BREcardModel();
            this.request = new BREcardModel();
            this.collection = new BREcardCollection();
            this.temp = new BREcardCollection();
        },


        bindModalKeys: function () {
            var _self = this;
            this.bindModalKeyEvents(window.PAGE_ID.BR_ONECARD_LOGIN, window.KEYS.Esc , function () {
                _self.hideModal(window.PAGE_ID.BILLING_RETURN);
                $('input[name = billingrt]').focus();
            });
            this.bindModalKeyEvents(window.PAGE_ID.BR_ONECARD_LOGIN, window.KEYS.Down, function() {
                _self.scrollDown();
            });
            this.bindModalKeyEvents(window.PAGE_ID.BR_ONECARD_LOGIN, window.KEYS.Up, function() {
                _self.scrollUp();
            });
            this.bindModalKeyEvents(window.PAGE_ID.BR_ONECARD_LOGIN, window.KEYS.Enter, function(){
            ...
            this.bindModalKeyEvents(window.PAGE_ID.BR_ONECARD_LOGIN, window.KEYS.Right, function() {
                _self.isCardLogin = true;
                $('#licard0').addClass('cus-selected');
            });
            this.bindModalKeyEvents(window.PAGE_ID.BR_ONECARD_LOGIN, window.KEYS.Left, function () {
                ...
                if(cardid != '' && password != ''){
                    ...
                    _self.request.vipinfo(data,function(resp) {
                        if(resp.status == '00'){
                            ...
                            _self.hideModal(window.PAGE_ID.BILLING_RETURN);
                            this.brecardpayview = new BREcardView(dataAccount);
                            _self.showModal(window.PAGE_ID.BR_ONECARD_PAY,_self.brecardpayview);
