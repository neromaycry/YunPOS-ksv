...
define([
    ...
    '../../moduals/modal-rtecardlogin/model',
    '../../moduals/modal-rtecardlogin/collection',
    '../../moduals/modal-rtecardpay/view',
    'text!../../moduals/modal-rtecardlogin/tpl.html',
    ...
    var ecardView = BaseModalView.extend({
        ...
        bindModalKeys: function () {
            var _self = this;
            this.bindModalKeyEvents(window.PAGE_ID.RT_ONECARD_LOGIN, window.KEYS.Esc , function () {
                _self.hideModal(window.PAGE_ID.BILLING_RETURN);
                $('input[name = billingrt]').focus();
            });
            this.bindModalKeyEvents(window.PAGE_ID.RT_ONECARD_LOGIN, window.KEYS.Down, function() {
                _self.scrollDown();
            });
            this.bindModalKeyEvents(window.PAGE_ID.RT_ONECARD_LOGIN, window.KEYS.Up, function() {
                _self.scrollUp();
            });
            this.bindModalKeyEvents(window.PAGE_ID.RT_ONECARD_LOGIN, window.KEYS.Enter, function(){
            ...
            this.bindModalKeyEvents(window.PAGE_ID.RT_ONECARD_LOGIN, window.KEYS.Right, function() {
                _self.isCardLogin = true;
                $('#licard0').addClass('cus-selected');
            });
            this.bindModalKeyEvents(window.PAGE_ID.RT_ONECARD_LOGIN, window.KEYS.Left, function () {
                ...
                if(cardid != '' && password != ''){
                    ...
                    _self.request.vipinfo(data,function(resp) {
                        if(resp.status == '00'){
                            ...
                            _self.showModal(window.PAGE_ID.RT_ONECARD_PAY,_self.ecardpayview);
